<template>
  <section class="pricing-config">
    ...
    <form v-if="!loadingPricing" @submit.prevent="savePricingConfig" class="pricing-config__form">
      ...
      <div class="pricing-panel">
        <div class="pricing-panel__title">2. GIÁ ĐẶC BIỆT</div>

        <div v-if="rulePageMode !== 'view' && rulePageMode !== 'edit'" id="special-pricing-rule-editor" class="rule-section">
          <div class="rule-section__topbar">
            <div class="rule-section__title">2.1 THÊM QUY TẮC</div>
            <button v-if="rulePageMode === 'create'" type="button" class="back-link" @click="backToRuleList">← Quay lại danh sách quy tắc</button>
          </div>
          ...
          <div class="pricing-grid">
            ...
            <div class="pricing-field">
              ...
              <select v-if="pricingRuleDraft.scope !== 'movie'" v-model="pricingRuleDraft.movie_id">
            ...
            <div class="pricing-field pricing-field--full date-exceptions">
              ...
              <div class="date-exceptions__input">
                ...
                <button type="button" class="btn-secondary" @click="addExcludedDate">+ Thêm ngày ngoại trừ</button>
              </div>
              <div v-if="pricingRuleDraft.excluded_dates.length" class="date-exceptions__list">
                <span v-for="date in pricingRuleDraft.excluded_dates" :key="date" class="date-exceptions__item">{{ formatDisplayDate(date) }} <button type="button" @click="removeExcludedDate(date)" aria-label="Xóa ngày ngoại trừ">X</button></span>
              </div>
              <span v-if="pricingRuleErrors.excluded_dates" class="error-msg">{{ pricingRuleErrors.excluded_dates[0] }}</span>
            </div>

            <div class="pricing-field pricing-field--time-scope">
              ...
              <div v-if="pricingRuleDraft.use_time_filter" class="time-range-grid">
            ...
            <div class="pricing-field pricing-field--status" v-if="false">
              <label>Trạng thái</label>
              <div class="status-options">
                <label class="status-option"><input v-model="pricingRuleDraft.status" type="radio" value="active" name="rule_status_create" /><span>Bật</span></label>
                <label class="status-option"><input v-model="pricingRuleDraft.status" type="radio" value="inactive" name="rule_status_create" /><span>Tắt</span></label>
              </div>
            </div>
          ...
          <div class="pricing-actions">
            ...
            <button type="button" class="btn-primary" @click="addPricingRule">Tạo quy tắc</button>
          </div>
        </div>

        <div v-if="rulePageMode !== 'view' && rulePageMode !== 'edit'" class="rule-section rule-section--list">
          <div class="rule-section__header">
            <div class="rule-section__title">2.2 QUẢN LÝ QUY TẮC</div>
          </div>
          ...
          <div class="rule-list">
            <div v-if="pricingForm.pricing_rules && pricingForm.pricing_rules.length" v-for="(rule, index) in pricingForm.pricing_rules" :key="index" class="rule-list__item">
              <div class="rule-list__content">
                ...
                <small>{{ rule.scope === 'movie' ? 'Theo phim' : 'Toàn hệ thống' }} • {{ rule.seat_type || 'Tất cả' }} • {{ rule.status === 'active' ? 'Bật' : 'Tắt' }}</small>
              </div>
              <div class="rule-list__meta">
                <span>{{ rule.adjustment_type === 'free' ? 'Miễn phí' : rule.value ? rule.value + 'đ' : '0đ' }}</span>
        ...
        <div v-else-if="rulePageMode === 'view'" class="rule-section">
          <div class="rule-section__topbar">
            <button type="button" class="back-link" @click="backToRuleList">← Quay lại danh sách quy tắc</button>
          </div>
          <div class="rule-section__title">Xem quy tắc giá</div>
          ...
          <div class="detail-grid">
            ...
            <div class="detail-row"><span class="detail-label">Ngày bắt đầu</span><span class="detail-value">{{ pricingRuleDraft.start_date || '—' }}</span></div>
            <div class="detail-row"><span class="detail-label">Ngày kết thúc</span><span class="detail-value">{{ pricingRuleDraft.end_date || '—' }}</span></div>
            ...
            <div class="detail-row"><span class="detail-label">Giá trị</span><span class="detail-value">{{ pricingRuleDraft.adjustment_type === 'free' ? 'Miễn phí' : `${pricingRuleDraft.value || 0} VNĐ` }}</span></div>
          ...
          <div class="pricing-actions">
            <button type="button" class="btn-secondary" @click="backToRuleList">Quay lại</button>
            <button type="button" class="btn-primary" @click="editRule(selectedRuleIndex)">Sửa</button>
          </div>
        </div>

        <div v-else-if="rulePageMode === 'edit'" id="special-pricing-rule-editor" class="rule-section rule-section--edit">
          <div class="rule-section__topbar">
            <button type="button" class="back-link" @click="backToRuleList">← Quay lại danh sách quy tắc</button>
          </div>
          <div class="rule-section__title">Sửa quy tắc giá</div>
          ...
          <div class="pricing-grid">
            ...
            <div class="pricing-field pricing-field--status">
              <label>Trạng thái</label>
              <div class="status-options">
                <label class="status-option"><input v-model="pricingRuleDraft.status" type="radio" value="active" name="rule_status_edit" /><span>Bật</span></label>
                <label class="status-option"><input v-model="pricingRuleDraft.status" type="radio" value="inactive" name="rule_status_edit" /><span>Tắt</span></label>
              </div>
            </div>
          ...
          <div class="pricing-actions">
            <button type="button" class="btn-secondary" @click="backToRuleList">Hủy</button>
            <button type="button" class="btn-primary" @click="saveEditedRule">Lưu thay đổi</button>
          </div>
        </div>
      </div>
...
const viewRule = async (index) => {
  ...
  await nextTick();
  const target = document.getElementById('special-pricing-rule-editor');
...
const editRule = async (index) => {
  ...
  const firstInput = document.querySelector('#special-pricing-rule-editor input');
