  ...
  <div class="payment-view" v-if="bookingStore.selectedSeats.length > 0">
    <div v-if="bookingSuccess" class="success-receipt-container glass-panel animate-fade-in">
      <div class="success-icon-wrapper">
        <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="0 0 24 24" fill="none" stroke="#00f5a0" stroke-width="2.5"><polyline points="20 6 9 17 4 12"></polyline></svg>
      </div>
      <h2 class="glow-text-mint">ĐẶT VÉ THÀNH CÔNG!</h2>
      <p class="success-subtitle">Cảm ơn bạn đã lựa chọn CineGo. Vé của bạn đã được xuất hệ thống.</p>
      
      <div class="ticket-receipt">
        <div class="receipt-header">
          <h3>MÃ ĐẶT VÉ: <span class="booking-code">{{ bookingCode }}</span></h3>
          <p>Xuất trình mã này tại quầy vé để nhận vé giấy</p>
        </div>
        
        <div class="receipt-body">
          <div class="receipt-row"><strong>Phim:</strong> <span>{{ bookingStore.selectedMovie?.title }}</span></div>
          <div class="receipt-row"><strong>Suất chiếu:</strong> <span>{{ bookingStore.selectedShowtime?.start_time }} | {{ bookingStore.selectedShowtime?.date }}</span></div>
          <div class="receipt-row"><strong>Phòng chiếu:</strong> <span>{{ bookingStore.selectedShowtime?.room_name }}</span></div>
          <div class="receipt-row"><strong>Ghế ngồi:</strong> <span class="seat-highlight">{{ bookingStore.selectedSeats.map(s => `${s.row}${s.number}`).join(', ') }}</span></div>
          <div class="receipt-row" v-if="bookingStore.selectedCombos.length > 0">
            <strong>Bắp nước:</strong> 
            <span>{{ bookingStore.selectedCombos.map(c => `${c.combo.name} (x${c.quantity})`).join(', ') }}</span>
          </div>
    ...
    <div v-else class="payment-checkout-grid">
      <!-- Cột trái: Chọn bắp nước & Phương thức thanh toán -->
          ...
          <div class="combos-list">
            <div v-for="combo in availableCombos" :key="combo.id" class="combo-item glass-panel">
              ...
              <div class="combo-controls">
                ...
                <button @click="bookingStore.addCombo(combo)" class="ctrl-btn">+</button>
              </div>
            </div>
          </div>
        </section>
      ...
      <!-- Cột phải: Summary & Áp voucher & Thanh toán -->
      <div class="checkout-sidebar glass-panel">
        ...
        <div class="invoice-details">
          <div class="invoice-group">
            ...
            <p class="invoice-meta">{{ bookingStore.selectedShowtime?.start_time }} | {{ bookingStore.selectedShowtime?.date }} | Phòng {{ bookingStore.selectedShowtime?.room_name }}</p>
          </div>
          ...
          <div class="invoice-row text-discount" v-if="bookingStore.discountAmount > 0">
            <span>Giảm giá:</span>
          ...
          <div class="invoice-row invoice-total">
            <span>Tổng cộng:</span>
            <span class="total-price glow-text-pink">{{ formatCurrency(bookingStore.totalAmount) }}</span>
          </div>
        </div>
...
const availableCombos = [
  ...
  },
  {
    ...
    price: 185000,
    image_url: 'https://images.unsplash.com/photo-1513558161293-cdaf765ed2fd?auto=format&fit=crop&w=150&q=80'
  }
];
...
const applyVoucher = async () => {
  ...
  try {
    const response = await api.post('/vouchers/verify', { code, subtotal: bookingStore.subtotal });
    ...
    console.warn('Voucher API not available. Simulating locally:');
    if (code === 'CINEGO10') {
      const mockVoucher = {
        code: 'CINEGO10',
      ...
      bookingStore.applyVoucher(mockVoucher);
      voucherMessage.value = 'Áp dụng mã CINEGOFREE giảm 100% thành công!';
      voucherSuccess.value = true;
    } else {
...
const submitBooking = async () => {
  ...
  try {
    const payload = {
      ...
      combos: bookingStore.selectedCombos.map(c => ({ id: c.combo.id, quantity: c.quantity })),
    ...
    try {
      ...
      console.warn('Booking API not fully integrated. Simulating success response.');
      responseData = {
        booking_code: 'CG-' + Math.floor(100000 + Math.random() * 900000)
      };
    }
    
    bookingCode.value = responseData.booking_code;
    bookingSuccess.value = true;
    
  } catch (err) {
    alert('Đặt vé không thành công. Suất chiếu có thể vừa bị đóng hoặc có lỗi kết nối!');
  } finally {
...
.combos-section, .payment-methods-section {
  padding: 30px;
}
...
.btn-apply-voucher, .btn-remove-voucher {
  padding: 10px 16px;
  border: none;
  font-weight: 700;
  font-size: 13px;
