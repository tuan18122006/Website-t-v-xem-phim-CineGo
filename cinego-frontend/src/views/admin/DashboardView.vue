    ...
    <main class="admin-main-content">
      <header class="content-header">
        ...
        <router-link to="/" class="btn-back-client"
          >👁️ Xem Client Website</router-link
        >
      </header>
            ...
            <div v-if="topMovies.length" class="movie-ranks-list">
              <div v-for="(m, i) in topMovies" :key="m.id" class="rank-item">
                <span
                  class="rank-num"
                  :class="
                    ['bg-pink', 'bg-violet', 'bg-tertiary'][i] || 'bg-tertiary'
                  "
                  >{{ i + 1 }}</span
                >
                <div class="rank-info">
          ...
          <table class="report-table">
            ...
            <tbody>
              <tr v-for="b in bookings" :key="b.id">
                ...
                <td>
                  <span class="method-badge">{{ b.payment_method }}</span>
                </td>
                <td>
                  <span
                    class="status-pill-small"
                    :class="{
                      active: b.payment_status === 'paid',
                      pending: b.payment_status === 'pending',
                    }"
                  >
                    {{
...
const getTabTitle = computed(() => {
  const titles = {
    ...
    blogs: "Quản Lý Blog",
    blogCategories: "Quản Lý Thể Loại Blog",
...
const getTabDesc = computed(() => {
  const descs = {
    ...
    combos:
      "Cấu hình giá cả, thêm/sửa/xóa và quản lý trạng thái hiển thị của các gói Bắp & Nước.",
    vouchers:
...
watch(activeTab, (val) => {
  blogMenuOpen.value = val === "blogs" || val === "blog-categories";
});
